...
{\LARGE MIE286 Lecture Notes}\\[0.4cm]
...
\textbf{Chapter 4}
\dotfill
\hyperref[chap:four]{\pageref*{chap:four}}
...
4.1 Expected Value
\dotfill
\hyperref[sec:expected_value]{\pageref*{sec:expected_value}}

\noindent\hspace{1.2cm}
4.1.1 Expected Value of a Random Variable
\dotfill
\hyperref[sec:ev_definition]{\pageref*{sec:ev_definition}}

\noindent\hspace{1.2cm}
4.1.2 Expected Value of a Function of a Random Variable
\dotfill
\hyperref[sec:ev_function]{\pageref*{sec:ev_function}}

\noindent\hspace{1.2cm}
4.1.3 Expected Value from a Joint Distribution
\dotfill
\hyperref[sec:ev_joint]{\pageref*{sec:ev_joint}}

\noindent\hspace{1.2cm}
4.1.4 Properties of Expected Value
\dotfill
\hyperref[sec:ev_properties]{\pageref*{sec:ev_properties}}

\noindent\hspace{0.5cm}
4.2 Variance
\dotfill
\hyperref[sec:variance]{\pageref*{sec:variance}}
...
\subsection*{Chapter 4 Review: Expected Value of a Function of a Random Variable}
\label{sec:ev_function}


\begin{enumerate}
\item \textit{\underline{Expected value of a function: expected average value in the long wrong}}, $E[g(X)]$  
...
\item \textbf{Example (continuous RV):}  
...
\subsection*{Expected Value from a Joint Distribution}
\label{sec:ev_joint}
...
\noindent
\textbf{Computing $E(X)$ from a joint PDF}
...
\noindent
\textbf{Method 1: Using the marginal distribution}
...
\noindent
\textbf{Method 2: Direct double integral}
...
\[
= \int_0^1 (1+3y^2)\left[ \frac{x^3}{3} \right]_0^2 dy
= \int_0^1 (1+3y^2)\frac{8}{3}\, dy
\]

\[
= \frac{8}{3} \int_0^1 (1+3y^2)\, dy
= \frac{8}{3} \cdot 2
= \frac{16}{3}
\]
...
\label{sec:ev_properties}


\textcolor{blue}{\textbf{Properties of Expectation}}

Let $X,Y$ be random variables and let $a,b \in \mathbb{R}$ be constants.

\begin{enumerate}
\item \textbf{Linearity (scaling and shifting):}
\[
\boxed{
E(aX + b) = aE(X) + b
}
...
\[
\boxed{
X \ge 0 \text{ and } E(X)=0 \iff P(X=0)=1
}
\]
\end{enumerate}
...
\noindent
$E(X)$ is a measure of the \textbf{center} of a distribution.

\medskip
\noindent
The \textbf{variance}, denoted $\sigma^2 = \mathrm{Var}(X)$, measures how closely the distribution is concentrated around the mean $\mu$.
...
\medskip
\noindent
$\sigma = \sqrt{\sigma^2}$ is called the \textbf{standard deviation}.
...
\begin{tcolorbox}[
    ...
    coltitle=black,
    colbacktitle=softpink,
    title=\textbf{Theorem (Variance Formula)},
    boxrule=0.8pt,
    arc=6pt,
    width=0.9\textwidth
]
\[
\mathrm{Var}(X) = \sigma^2 = E(X^2) - \mu^2
\]
\end{tcolorbox}
\end{center}
...
\noindent
\textbf{Variance of a function of a random variable}
...
\textcolor{blue}{\textbf{Properties of Variance}}
...
\begin{tcolorbox}[
    ...
    coltitle=black,
    colbacktitle=softpink,
    title=\textbf{Theorem (Variance of a Linear Combination)},
    boxrule=0.8pt,
    arc=6pt,
    width=0.9\textwidth
]
Let $X,Y$ be random variables with joint distribution $f(X,Y)$.
Then
\[
\mathrm{Var}(aX+bY)
= a^2\mathrm{Var}(X)
...
\medskip
\noindent
where the \textbf{covariance} is defined as
\[
\boxed{
\mathrm{Cov}(X,Y)
= E[(X-E[X])(Y-E[Y])]
...
\begin{itemize}
\item $\mathrm{Cov}(X,Y) > 0$: $X$ and $Y$ tend to increase together  
\item $\mathrm{Cov}(X,Y) < 0$: one increases while the other decreases  
\item $\mathrm{Cov}(X,Y) = 0$: no linear relationship
\end{itemize}

\medskip
\noindent
\textbf{Example (population intuition):}

If $X$ is height and $Y$ is weight in a population,
\[
(X-E[X])(Y-E[Y]) > 0
\]
for tall–heavy and short–light individuals, so $\mathrm{Cov}(X,Y) > 0$.
...
\begin{enumerate}
    \item \underline{\textit{Expected Value}}
    ...
    \item \underline{\textit{Variance}}

    \begin{itemize}
        \item Definition:
        \[
        \mathrm{Var}(X) = \sigma^2 = E\big[(X-\mu)^2\big]
        \]
    ...
    \item \underline{\textit{Variance of a Linear Combination}}
...
\begin{tcolorbox}[
    ...
    colframe=softpink,
    coltitle=black,
    title=\textbf{Theorem: Independence and Expected Value},
    boxrule=0.8pt,
    arc=6pt,
    width=0.9\textwidth
]
...
\[
E(XY)
= \left(\int_{-\infty}^{\infty} x g(X)\,dx\right)
\left(\int_{-\infty}^{\infty} y h(Y)\,dy\right)
\]
...
\textbf{Given:} The joint PMF table below.

\[
\begin{array}{c|ccc|c}
f(X,Y) & X=-1 & X=0 & X=1 & h(Y) \\ \hline
Y=-1 & \tfrac18 & \tfrac18 & \tfrac18 & \tfrac38 \\
Y=0  & \tfrac18 & 0         & \tfrac18 & \tfrac28 \\
Y=1  & \tfrac18 & \tfrac18 & \tfrac18 & \tfrac38 \\ \hline
g(X) & \tfrac38 & \tfrac28 & \tfrac38 & 1
\end{array}
\]

\begin{enumerate}
    \item \underline{\textit{Compute $E(X)$}}
    ...
    \]
    \[
    E(X)=(-1)\cdot\tfrac38 + 0\cdot\tfrac28 + 1\cdot\tfrac38
    =-\tfrac38 + 0 + \tfrac38
    =0
    \]

    \item \underline{\textit{Compute $E(Y)$}}
    ...
    \]
    \[
    E(Y)=(-1)\cdot\tfrac38 + 0\cdot\tfrac28 + 1\cdot\tfrac38
    =-\tfrac38 + 0 + \tfrac38
    =0
    \]

    \item \underline{\textit{Compute $E(XY)$}}
    ...
    \textbf{Row $y=-1$:}
    \[
    (-1)(-1)\tfrac18 + (0)(-1)\tfrac18 + (1)(-1)\tfrac18
    =\tfrac18 + 0 - \tfrac18 =0
    \]

    \textbf{Row $y=0$:} all terms are $0$ because $y=0$.
...
\medskip
\textbf{Note:} Even if $E(XY)=E(X)E(Y)$, this \underline{does not automatically mean} $X$ and $Y$ are independent.
...
\[
\mathrm{Cov}(X,Y)=0
\quad \text{does NOT necessarily imply independence.}
\]
...
\[
\Rightarrow \quad X \text{ and } Y \text{ are \underline{not independent}.}
\]

\bigskip

\textcolor{blue}{\textbf{Properties of Covariance}}
...
The covariance $\sigma_{XY}$ is \underline{not scale-free}.  
Its magnitude does \underline{not} directly indicate the strength of the linear relationship between $X$ and $Y$.

\medskip

\textbf{Definition: Correlation Coefficient}

The \underline{correlation coefficient} of $X$ and $Y$ is
\[
\boxed{
\rho_{XY}
=
...
where
\[
\sigma_X=\sqrt{\mathrm{Var}(X)}, \qquad
...
\begin{tcolorbox}[
    ...
    colframe=softpink,
    coltitle=black,
    title=\textbf{Theorem: Correlation Is Scale-Free},
    boxrule=0.8pt,
    arc=6pt,
    width=0.9\textwidth
]
...
Therefore, the correlation coefficient is \underline{scale-free}.
\end{tcolorbox}
\end{center}
...
\rho(aX+b,\;cY+d)
=
\frac{\mathrm{Cov}(aX+b,\;cY+d)}
{\sqrt{\mathrm{Var}(aX+b)\,\mathrm{Var}(cY+d)}}.
\]
...
\rho(aX+b,\;cY+d)
=
\frac{ac\,\mathrm{Cov}(X,Y)}
{\sqrt{a^2\mathrm{Var}(X)\,c^2\mathrm{Var}(Y)}}.
\]
...
\begin{tcolorbox}[
    ...
    colframe=softpink,
    coltitle=black,
    title=\textbf{Theorem: Bounds on Correlation},
    boxrule=0.8pt,
    arc=6pt,
    width=0.9\textwidth
]
...
\subsection*{Interpretation of the Correlation Coefficient}

\begin{enumerate}
    \item
    $\rho$ is a measure of the \underline{strength and direction}
    of the \underline{linear relationship} between $X$ and $Y$.

    \item
    When there is an \underline{exact linear relationship},
    \[
    \boxed{
    Y = aX + b
    }
...
% Support region for: 0<x<1 and x<y<x+1
...
\draw[->] (0,-0.05) -- (0,2.25) node[above] {$y$};

% Key x-boundaries
\draw[dashed] (0,0) -- (0,2.1) node[above left] {$x=0$};
\draw[dashed] (1,0) -- (1,2.1) node[above] {$x=1$};

% Lines y = x and y = x+1 (only drawn over 0<x<1)
\draw[thick] (0,0) -- (1,1) node[right] {$y=x$};
\draw[thick] (0,1) -- (1,2) node[right] {$y=x+1$};

% Shaded support region
\fill[babyblue!20, opacity=0.7] 
  (0,0) -- (1,1) -- (1,2) -- (0,1) -- cycle;

% Corner points (optional, helps clarity)
\fill (0,0) circle (0.015);
\fill (1,1) circle (0.015);
\fill (0,1) circle (0.015);
\fill (1,2) circle (0.015);

% Labels (optional)
\node at (0.45,0.75) {$f(X,Y)=1$};

% A few axis ticks (optional)
\draw (1,0) -- (1,-0.02) node[below] {$1$};
\draw (0,1) -- (-0.02,1) node[left] {$1$};
\draw (0,2) -- (-0.02,2) node[left] {$2$};
...
\[
E(X)=\int_{0}^{1}\int_{x}^{x+1} x\,f(X,Y)\,dy\,dx
=
\int_{0}^{1}\int_{x}^{x+1} x\,dy\,dx
\]
...
\[
E(Y)=\int_{0}^{1}\int_{x}^{x+1} y\,f(X,Y)\,dy\,dx
=
\int_{0}^{1}\int_{x}^{x+1} y\,dy\,dx
\]
...
\subsubsection*{Compute $E(XY)$}

\[
E(XY)=\int_{0}^{1}\int_{x}^{x+1} xy\,f(X,Y)\,dy\,dx
=
\int_{0}^{1}\int_{x}^{x+1} xy\,dy\,dx
\]

\[
=\int_{0}^{1}
x\left[\frac{y^{2}}{2}\right]_{x}^{x+1}
dx
=
\int_{0}^{1}
x\cdot\frac{(x+1)^2-x^2}{2}
dx
\]
...
\[
E(X^2)=\int_{0}^{1}\int_{x}^{x+1} x^2\,dy\,dx
=\int_{0}^{1} x^2\,dx
=\frac13
\]

\[
\mathrm{Var}(X)=\frac13-\left(\frac12\right)^2=\frac{1}{12}
\]

\[
\boxed{\mathrm{Var}(X)=\frac{1}{12}}
\]
...
\[
E(Y^2)=\int_{0}^{1}\int_{x}^{x+1} y^2\,dy\,dx
=\int_{0}^{1}
\left[\frac{y^3}{3}\right]_{x}^{x+1}
dx
\]
...
\[
\mathrm{Var}(Y)=\frac{7}{6}-1=\frac16
\]

\[
\boxed{\mathrm{Var}(Y)=\frac16}
\]

\subsubsection*{Variances (again but using $E[(X-\mu)^2]$}
...
dx
=
\left[\frac{x^3}{3}-\frac{x^2}{2}+\frac{x}{4}\right]_{0}^{1}
=\frac{1}{12}
\]
...
=
\int_{0}^{1}
\left[\frac{(y-1)^3}{3}\right]_{y=x}^{y=x+1}
dx
=
\int_{0}^{1}
\frac{(x)^3-(x-1)^3}{3}
...
=
\int_{0}^{1}
\left(x^2-x+\tfrac13\right)
dx
=
\left[\frac{x^3}{3}-\frac{x^2}{2}+\frac{x}{3}\right]_{0}^{1}
=\frac{1}{6}
\]

\[
\boxed{\mathrm{Var}(Y)=\frac{1}{6}}
\]
...
{\sqrt{\mathrm{Var}(X)\mathrm{Var}(Y)}}
=
\frac{\frac{7}{12}-\frac12}
{\sqrt{\frac{1}{12}\cdot\frac16}}
=
\frac{1}{\sqrt{2}}
\]
...
% Axes
\draw[->] (-0.05,0) -- (1.1,0) node[right] {$x$};
\draw[->] (0,-0.05) -- (0,1.25) node[above] {$y$};
...
% Lines
\draw[thick] (0,0) -- (1,1) node[right] {$y=x$};
\draw[thick] (0,0.1) -- (1,1.1) node[right] {$y=x+\tfrac{1}{10}$};

% Shaded region
\fill[babyyellow!25, opacity=0.8]
  (0,0) -- (1,1) -- (1,1.1) -- (0,0.1) -- cycle;

% Label
\node at (0.55,0.65) {$f(x,y)=10$};

% Ticks
\draw (1,0) -- (1,-0.02) node[below] {$1$};
\draw (0,0.1) -- (-0.02,0.1) node[left] {$\tfrac{1}{10}$};
...
\textbf{Calculations:}
\\
\textbf{Variance of a Uniform random variable:}

\[
\boxed{
\operatorname{Var}(Z)=\frac{(b-a)^2}{12}
\quad \text{for } Z\sim \text{Uniform}(a,b)
}
\]
...
\]
\[
\operatorname{Var}(X)=\frac{(1-0)^2}{12}=\frac{1}{12}.
\]

\[
\operatorname{Var}(U)=\frac{(1/10 - 0)^2}{12}=\frac{1}{1200}.
\]
...
\[
\operatorname{Var}(Y)=\operatorname{Var}(X)+\operatorname{Var}(U)
=\frac{1}{12}+\frac{1}{1200}
=\frac{101}{1200}.
\]
...
=\operatorname{Cov}(X,X+U)
=\operatorname{Cov}(X,X)+\operatorname{Cov}(X,U).
\]

Since \(\operatorname{Cov}(X,U)=0\),
\[
\operatorname{Cov}(X,Y)=\operatorname{Var}(X)=\frac{1}{12}.
\]
...
=
\frac{\frac{1}{12}}
{\sqrt{\left(\frac{1}{12}\right)\left(\frac{101}{1200}\right)}}
=
\sqrt{\frac{100}{101}}.
\]
...
\begin{itemize}
\item A \textbf{smaller band width} means \(Y\) stays closer to the line \(y=x\).
\item This implies less variation in \(Y-X\).
\item Hence, \(Y\) is more tightly determined by \(X\).
\end{itemize}
...
\textbf{Key idea:}
A thinner diagonal support band implies stronger linear dependence and higher correlation between \(X\) and \(Y\).

\section*{Feb 6 }

\subsection*{Discrete Uniform Distribution}

A random variable $X$ has a \underline{discrete uniform distribution} if it assumes the values
\[
x_1, x_2, \dots, x_k
\]
...
= \frac{1}{k} \sum_{i=1}^{k} (x_i - \mu)^2
}
\]

\subsection*{Bernoulli Distribution}

A \underline{Bernoulli random variable} has only two outcomes:
\begin{itemize}
    \item success
    \item failure
\end{itemize}
...
For example, in a cancer experiment, success could be defined as death.)
...
\subsection*{Example: Independent Bernoulli Trials}

Four components are tested independently.  
...
The \underline{binomial distribution} gives the number of successes in $n$ independent Bernoulli trials.
...
b(x;n,p) = f(X)
= \binom{n}{x} p^x q^{\,n-x},
\qquad x = 0,1,2,\dots,n
}
\]

\subsection*{Binomial Expansion Check}
...
\subsection*{Mean of a Binomial Random Variable}
...
I_i =
\begin{cases}
1, & \text{success on trial } i \\
0, & \text{failure on trial } i
\end{cases}
\]
...
\subsection*{Variance of a Binomial Random Variable}
...
\[
\operatorname{Cov}(I_i, I_j) = 0 \quad (i \neq j)
\]

\[
\operatorname{Var}(X)
= \operatorname{Var}(I_1 + \cdots + I_n)
\]
...
\subsection*{Example: Testing a Manufacturer's Claim}

A manufacturer claims that the defective rate is $p = 0.1$.
We test $n=4$ components independently and observe $3$ defectives.

Let
\[
X = \text{number of defectives in } 4 \text{ trials}
\]
...
\textbf{Important note:}  
The claim is either true or false; there is no probability attached to the claim itself.
The probability calculation assumes the claim is true and measures how surprising the observed data would be under that assumption.

(We will study this idea formally in Chapter 9.)

\subsection*{Multinomial Distribution}

The \underline{multinomial distribution} applies when each trial has more than two possible outcomes.

\medskip

\textbf{Definition:}  
Suppose a single trial can result in $k$ outcomes
\[
E_1, E_2, \dots, E_k
\]
with probabilities
\[
p_1, p_2, \dots, p_k,
\qquad \sum_{i=1}^{k} p_i = 1
\]

Let
\[
X_i = \text{number of occurrences of outcome } E_i
\]
...
\[
\boxed{
f(x_1, x_2, \dots, x_k; p_1, \dots, p_k, n)
...
\textbf{Interpretation:}  
This is like Bernoulli trials, but with more than two possible outcomes per trial.

\subsection*{Example: Travel Choice (Multinomial)}
...
Let
\[
X_1 = \text{number of days taking the bus}
\]
\[
X_2 = \text{number of days taking the train}
\]
\[
X_3 = \text{number of days taking the car}
\]
...
\subsection*{Example: Traffic Light (Multinomial)}
...
Define the random variables:
\[
X_1 = \text{number of times the light is green}
\]
\[
X_2 = \text{number of times the light is yellow}
\]
\[
X_3 = \text{number of times the light is red}
\]
...
\begin{enumerate}
    \item \textit{\underline{Discrete uniform distribution}}  
    A discrete random variable that takes $k$ possible values with equal probability.

    \item \textit{\underline{Bernoulli distribution}}  
    A discrete random variable with two outcomes: success or failure.

    \item \textit{\underline{Binomial distribution}}  
    The number of successes in $n$ independent Bernoulli trials.

    \item \textit{\underline{Multinomial distribution}}  
    A generalization of the binomial distribution with more than two outcomes per trial.
\end{enumerate}

\subsection*{Hypergeometric Distribution}

The \underline{hypergeometric distribution} models \textbf{sampling without replacement}.
Therefore, the trials are \textbf{dependent}.
...
\begin{itemize}
    ...
    \item $n$ = sample size
    \item $X$ = number of successes in the sample
\end{itemize}
...
P(X=2)
=
\frac{\binom{7}{2}\binom{5}{1}}{\binom{12}{3}}
\]

\[
P(X=x)
=
\frac{\binom{7}{x}\binom{5}{3-x}}{\binom{12}{3}}
\]
...
\begin{itemize}
    \item Each batch contains $N=15$ compressors.
    \item A batch is \underline{unacceptable} if it contains \textbf{two or more faulty compressors}.
\end{itemize}

To inspect a batch:
\begin{itemize}
    \item A random sample of $n=5$ compressors is selected.
    \item All sampled compressors are tested.
    \item If \textbf{none are defective}, the batch is accepted.
\end{itemize}
...
\[
X = \text{number of faulty compressors in the sample of 5}
\]

Since sampling is done \underline{without replacement}, $X$ follows a hypergeometric distribution:
\[
X \sim \text{Hypergeometric}(N=15,\; k=2,\; n=5)
\]

\begin{itemize}
    \item $N=15$: total compressors in the batch
    \item $k=2$: faulty compressors in the batch
    \item $n=5$: compressors tested
\end{itemize}

\medskip

\textbf{Probability of accepting a bad batch:}  
...
P(X=0)
=
\frac{\binom{2}{0}\binom{13}{5}}{\binom{15}{5}}
= 0.428
\]
...
This inspection plan has a high chance of accepting bad batches and is therefore \underline{not a good plan}.

\subsection*{Mean and Variance of the Hypergeometric Distribution}
...
\frac{N-n}{N-1}
\]
is called the \underline{finite population correction factor} and appears because sampling is done without replacement.
...
In this case, a \underline{binomial distribution} can be used to approximate the hypergeometric distribution with
\[
\boxed{
p = \frac{k}{N}
}
...
\textcolor{blue}{\textbf{Rule of thumb:}}
\[
\frac{n}{N} \ll 0.05
\]

\subsection*{Example: Binomial Approximation to Hypergeometric}
...
Let
\[
X = \text{number of blemished tires in the sample}
\]


\textbf{Exact (hypergeometric) probability:}

\[
P(X=3)
=
\frac{\binom{1000}{3}\binom{4000}{7}}{\binom{5000}{10}}
= 0.2014
\]
...
The \underline{negative binomial distribution} models the number of trials required for the
\textbf{$k$th success} to occur in a sequence of independent Bernoulli trials.
...
Let
\[
X = \text{trial on which the $k$th success occurs}
\]
...
\begin{itemize}
    \item The $k$th success must occur on trial $x$
    \item In the first $x-1$ trials, there must be exactly $k-1$ successes
    \item Trial $x$ must be a success
\end{itemize}
...
\subsection*{Interpretation (intuition)}



\textbf{Application (disease screening):}

Suppose people are tested one by one for a disease.
Each person independently tests positive with probability $p$.

Testing continues until $k$ infected individuals have been identified.
Let
\[
X = \text{number of people tested until the $k$th infected person is found}
\]
...
Electronic components are tested independently.
...
Let
\[
X = \text{trial on which the 3rd defective component is found}
\]


\textbf{Probability that the 3rd defective occurs on the 4th trial:}

\[
P(X=4)
=
\binom{3}{2}(0.1)^3(0.9)^1
\]

\medskip

\textbf{Probability that the 3rd defective occurs on the 5th trial:}

\[
P(X=5)
=
\binom{4}{2}(0.1)^3(0.9)^2
\]

\subsection*{PMF of the Negative Binomial Distribution}

Let $X$ be the trial on which the $k$th success occurs in a sequence of independent Bernoulli trials.
Each trial has success probability $p$ and failure probability $q=1-p$.
...
\subsection*{Interpretation of the Mean}

The mean $\mu = k/p$ represents the \underline{average number of trials required} to obtain $k$ successes.

For example, in a disease-screening context:
\begin{itemize}
    \item $k$ is the number of infected individuals you want to identify
    \item $p$ is the probability that a randomly tested person is infected
\end{itemize}
...
The \underline{geometric distribution} models the number of trials required for the
\textbf{first success} to occur in a sequence of independent Bernoulli trials.

\medskip

Let
\[
X = \text{trial on which the first success occurs}
\]
...
\subsection*{Relation to the Negative Binomial}

The geometric distribution is a \underline{special case} of the negative binomial distribution with
\[
k=1
\]

That is, it counts the waiting time until the \textbf{first} success.
...
Let $X$ follow a geometric distribution.
For integers $s>t$,
\[
P(X>s \mid X>t) = P(X>s-t)
\]

This property is called the \underline{memoryless property}.
\end{tcolorbox}
\end{center}

\subsection*{Interpretation}

In gambling, the probability of losing or winning the next game does \underline{not} depend on the fact that you have already lost five games in a row.
